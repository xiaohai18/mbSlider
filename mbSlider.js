    ...
    mbSlider.prototype = {
        ...
        */
        init: function(config){
            ...
            this.timer          = null;
            this.boxViewSize    = config.direction == 'vertical' ? this.container.clientHeight || this.container.offsetHeight : this.container.clientWidth || this.container.offsetWidth;
            ...
            this.distance = -this.boxViewSize;
        ...
        /*
            设置运动
        */
        setMove: function(distance){
            var config = this.config; 

            if(this.duration){config.aniStart()};
            this.setAttributes('transform', this.config.direction == 'vertical' ? 'translate3d(0px, '+distance+'px, 0px)' : 'translate3d('+distance+'px, 0px, 0px)');
        },
        ...
        */
        setState: function (){
            var curIndex = this.getCurIndex(), self = this;
            self.setTransitionDuration(self.config.duration);

            if(curIndex < -self.itemSize || curIndex > -1){
                self.setMove(self.distance);
                curIndex < -self.itemSize && (self.distance = -self.boxViewSize);
                curIndex > -1 && (self.distance = -self.boxViewSize*self.itemSize);
            ...
            if(self.config.isPages){
                curIndex = Math.abs(self.getCurIndex()%self.itemSize);
        ...
        */
        autoplay: function(){
            ...
            self.timer = setInterval(function(){
                self.distance -= self.boxViewSize;
                self.setState();
        ...
        */
        bindTouchEvent: function(){
            ...
            function move(e){
                ...
                self.setMove(self.distance + b);
                self.config.tapMove.call(container, e);
            }

            function end(){
                if(Math.abs(b) >= self.boxViewSize/2){
                    self.distance = self.distance + (b > 0 ? self.boxViewSize : -self.boxViewSize)
                }
                self.setTransitionDuration(self.config.duration);
                self.setState();
                self.autoplay();
        ...
        */
        ...
        bindPagesEvent: function(){
            var self = this, fn = function(e){
                ...
                self.distance = -(this.index+1) * self.boxViewSize;
                self.setTransitionDuration(self.config.duration);
                self.setState();
                self.autoplay();
            }
        ...
        */
        showControl: function(){
            ...
            var html = [
                '<a href="" class="mb-control mb-control-pre"></a>',
                '<a href="" class="mb-control mb-control-next"></a>'
            ];
